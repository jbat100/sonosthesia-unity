...
namespace Sonosthesia
{
    ...
    public class BaseTransModifier<TRepresentation> : BaseModifier<TRepresentation> where TRepresentation : class
    {
        public ITransAccessor<TRepresentation> transAccessor;
        ...
        public override void ApplyParameter(TRepresentation representation, string key, IList<float> parameter)
        {
            Trans current = transAccessor.GetTrans(representation);

            Trans modified = _implementation.Modify(key, settings, current, parameter);

            transAccessor.SetColor(representation, modified);
        }
    }
